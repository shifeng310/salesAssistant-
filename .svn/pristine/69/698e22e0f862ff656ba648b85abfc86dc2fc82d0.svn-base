...
@synthesize leftSideBarViewController,rightSideBarViewController,contentView,navBackView,selectIndex;

//声明静态变量和常量
static SidebarViewController *rootViewCon;
const int ContentOffset=260;
...
- (void)viewDidLoad
{
    ...
    _leftCon._selectIdnex = selectIndex;
...
- (void)panInContentView:(UIPanGestureRecognizer *)panGestureReconginzer
{
	...
	if (panGestureReconginzer.state == UIGestureRecognizerStateChanged)
    {
        ...
        {
            if(translation+currentTranslate>260)
            {
                return;
            }
